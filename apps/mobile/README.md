...
- Workout history export is available from the History header and through Profile → Account & Data. Users can export completed workouts from the last 7, 30, or 90 days, or all time. Both formats omit internal IDs, generation metadata, planned targets, and other implementation fields. JSON keeps a clean nested training record; CSV provides one row per set. Optional duration is included only for timed exercise results. Files are created in the app cache and handed to the device share menu.
