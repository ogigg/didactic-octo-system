...
struct RestTimerView: View {
    ...
    var body: some View {
        ...
        WatchScreen(title: rest?.isPaused == true ? String(localized: "Paused") : String(localized: "Rest"),
            ...
            onPrimary: { if remaining == 0 { coordinator.skipRest() } else { skipConfirmation = true } }) {
            VStack(spacing: 2) {
                ...
                HStack(spacing: 0) {
                    Button { coordinator.adjustRest(by: -15) } label: { Text("−15s").frame(maxWidth: .infinity, minHeight: 28) }
                    Button {
