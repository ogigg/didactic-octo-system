...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabaseClient
      .from("profiles")
      .select(
        "goal, custom_goal, weekly_frequency, gender, training_split, session_duration_minutes, equipment_level, training_style, difficulty_level, training_custom_prompt, weight_increment_kg, weight_micro_increment_kg, training_setup_completed"
      )
      .eq("id", user_id)
      .single();
