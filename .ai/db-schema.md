...
- `weight_increment_kg`, `weight_micro_increment_kg`: optional user-configured load steps (e.g. `4` for a pin-loaded machine, `1.1` for magnetic micro-plates). NULL means auto — the progression engine falls back to equipment-based defaults. Reachable increases are combinations of `n × weight_increment_kg + m × weight_micro_increment_kg`; both the progression engine and the LLM prompt use them so suggested loads are always physically settable.
