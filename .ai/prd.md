...
### 3.3a. Training program lifecycle

- Training preferences (split, duration, equipment, style, difficulty, optional focus prompt) are configured once via the "Set Up Your Training" screen, accessible after onboarding.
- The selected training split determines the workout focus rotation: Full Body repeats the same balanced focus; Upper/Lower alternates; Push/Pull/Legs cycles through three foci.
- After each completed workout, the backend auto-generates the next workout using saved preferences + recent workout history + difficulty feedback for progressive overload.
- Users can reconfigure preferences at any time from settings, which takes effect on the next generated workout.
- The `training_setup_completed` flag on the profile controls whether the setup screen should be shown.
